...
export class BlockPolledLiquityStore extends LiquityStore<BlockPolledLiquityStoreExtraState> {
  ...
  private async _get(
    ...
    const {
      ...
      marketPrice: this._readable.getMarketPrice({ blockTag }),
      piRedemptionRate: this._readable.getRedemptionRate({ blockTag }),
      deviationFactor: this._readable.getDeviationFactor({ blockTag }),
